  ...
  useEffect(() => {
    async function fetchPlanilha() {
      try {
        ...
        console.log("Colunas encontradas:", headerRaw);
        const exampleRow = lines[1]?.split(",");
        console.log("Exemplo de linha:", exampleRow);

        const idxNick = header.findIndex(h => h.includes("nome"));
        const idxLevel = header.findIndex(h => h.includes("level"));
        const idxVocation = header.findIndex(h => h.includes("vocação") || h.includes("vocation"));
